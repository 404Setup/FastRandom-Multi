...
public class FasterRandomConfig {
	...
	private static ConfigPatchSet readConfigFile(File file) {
		...
		try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
			...
			if (EngineName == null) {
				throw new Exception(
						"Unacceptably broken config detected! Please delete this config to re-generate a working one! Using default values.");
			}

			if (mergeRand || !EngineName.equals("L64X128MixRandom")) {
				UNSUPPORTED_SETTINGS_ENABLED = true;
				configLogger.warn(
						"Unsupported settings have been enabled! When reporting an issue, consider disabling this to see if anything remains broken before making an issue.");
